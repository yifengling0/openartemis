...
| `OA_PROFILE` | **帧性能基线**（P1 profiler）：每 5 s 打印一行 `[prof]` 差分——帧率、draws/batches/binds per frame、纹理新建数、uploads/frame、upMB/s、decode 次数与 ms、readMB/s、缺失缓存命中数、层数。窗口线（含 OHOS 真机）有像素数据；headless 无后端，渲染项为 0 | src/app/main.cpp、src/core/render/backend*.cpp、renderer.cpp | `OA_PROFILE=1 ./openartemis --fps 30 --frames 200 a.pfs` |
